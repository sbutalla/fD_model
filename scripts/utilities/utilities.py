...
resultDir   = "dataframes/" # base dir for all other subdirs which contain info/plots/data
...
class process_data:
    ...
    def __init__(self, dataset, ml_met = False):
        ...
        if ml_met and self.dataset not in ["mc", "sig"]: # only mc/sig samples have generator level information
            self.ml_met = ml_met # store ml_met bool as attribute
        elif ml_met and self.dataset not in ["mc", "sig"]:
            print_error("Can only use 'mc' or 'sig' datasets when extracting and saving observables used in computing the machine learning metrics.")
            sys.exit()
    ...
    def extract_data(self, root_file, root_dir, ret = False, verbose = False):
        ...
        if self.dataset == "ntuple":
            ...
            folder = file[cutflow_dir] # ntuples have the tree in a directory, so first get the contents of the directory 
        ...
        if ret:
            if self.dataset == "mc" or self.ml_met:
                if verbose:
                    print_alert('Arrays returned: selMu_eta, selMu_phi, selMu_pt, selMu_charge, genAMu_eta, genAMu_phi, genAMu_pt, genAMu_charge')
            ...
            elif self.dataset == "bkg" or self.dataset == "sig" or self.dataset == "ntuple":
                if verbose:
                    print_alert('Arrays returned: selMu_eta, selMu_phi, selMu_pt, selMu_charge')
    ...
    def prelim_cuts(self, ret = False, verbose = False):
        ...
        if verbose:
            ...
            print_alert(60 * '*')
            print_alert("\n\n")
            print_alert("Determing the events that were not properly reconstructed or are outside of the geometric acceptance of the detector system (eta > 2.4)")
        ...
        badSelCharge = np.unique(np.where(np.sum(self.selMu_charge, axis = 1) != 0)) # Remove event if the sum of the charge of all four muons isn't 0.
        ...
        if self.dataset == "bkg" or self.dataset == "sig" or self.dataset == "ntuple" and self.ml_met:
        ...
        if verbose:
            ...
            print_alert('Total number of events failing reconstruction in phi: %d' % len(badPhi))
            print_alert('Total number of events with sel eta > 2.4: %d' % len(badSelEta))

            if self.dataset == "mc" or self.ml_met:
                print_alert('Total number of events with gen eta > 2.4: %d' % len(badGenAEta))

            print_alert('Total number of events with sel pT == -100: %d'.format(len(badSelpT)))
            print_alert('Total number of events failing charge reconstruction: {}'.format(len(badCharge)))
        ...
        if ret:
            if self.dataset == "mc" or self.ml_met:
                if verbose:
                    print_alert("Arrays returned: selMu_etaCut, selMu_phiCut, selMu_ptCut, selMu_chargeCut, genAMu_etaCut, genAMu_phiCut, genAMu_ptCut, genAMu_chargeCut")
                ...
                if verbose:
                    print_alert("Arrays returned: selMu_etaCut, selMu_phiCut, selMu_ptCut, selMu_chargeCut")
    ...
    def matchBkgMu(self, ret = False, verbose = False):
        ...
        for event in range(numEvents):
            tempSelCharge = self.selMu_chargeCut[event, :] # (1 x 4) extract charge data for all 4 muons and assign to temp array
            self.min_dRgenFinal[event, :] = np.array(np.where(self.selMu_chargeCut[event, :])) # 1 x 4 THIS DOESN'T LOOK RIGHT...INVESTIGATE FURTHER
            if tempSelCharge[0] == tempSelCharge[1]: # check if muons 0 and 1 have the same charge, if so, swap first with third muon
        ...
        if np.sum(self.min_dRgenFinal, axis = 1).all() == np.full((numEvents), 6.).all(): # check to make sure "matching" performed properly
            ...
            if np.sum(self.min_dRgenFinal, axis = 1).all() == np.full((numEvents), 6.).all(): # check again to make sure "matching" performed properly
        ...
        for ii in tqdm(range(self.genAMu_etaCut.shape[0])): # for each event
            for jj in range(4): # dR for each gen muon
                for ll in range(4): #for each sel muon
                    self.dRgen[ii, jj, ll] = sqrt(pow(self.genAMu_etaCut[ii, jj] - self.selMu_etaCut[ii, ll], 2)+ pow(self.genAMu_phiCut[ii, jj] - self.selMu_phiCut[ii, ll], 2)) # calculate delta R between each gen and sel level muons
        if ret:
            return self.dRgen
    ...
    def SSM(self, ret = False, verbose = False, extraInfo = False):
        ...
        if verbose:
            print_alert("\n\n")
            print_alert(60 * '*')
            print_alert("Using tochastic sampling method (SSM) to match gen and reco level muons")
            print_alert(60 * '*')
            print_alert("\n\n")

        for event in tqdm(range(self.selMu_chargeCut.shape[0])): # loop over events
                
            tempGenCharge = self.genAMu_chargeCut[event, :] # (1 x 4) Pulls out the generator level charge for all four muons for one event
            tempSelCharge = self.selMu_chargeCut[event, :]  # (1 x 4) Pulls out the generator level charge for all four muons for one event
            ...
            if extraInfo:
                ...
                for muon in range(4):
                    dEtaMatched[event, muon] = self.genAMu_etaCut[event, genInd[muon]]  - self.selMu_etaCut[event, selInd[muon]]
                    dPhiMatched[event, muon] = self.genAMu_phiCut[event, genInd[muon]]  - self.selMu_phiCut[event, selInd[muon]]
                    dPtMatched[event, muon]  = self.genAMu_ptCut[event,  genInd[muon]]  - self.selMu_ptCut[event,  selInd[muon]]
        ...
        if ret:
            if not extraInfo:
                ...
                if verbose:
                    print_alert("Arrays returned: min_dRgen, dRgenMatched, dEtaMatched, dPhiMatched, dPtMatched")
    ...
    def dRcut(self, dRcut = dR_cut, ret = False, verbose = False):
        ...
        if verbose:
            print_alert("\n\n")
            print_alert(60 * '*')
            print_alert("Cutting events that do not match the criterion: dR < %.2f" % dRcut)
            print_alert(60 * '*')
            print_alert("\n\n")

        self.dRcut_events = np.unique(np.where(self.dRgenMatched > dRcut)[0]) # get (unique) indices where the condition is satisfied
    ...
    def permutations(self, ret = False, verbose = False):
        ...
        numEvents      = self.min_dRgenFinal.shape[0]                             # int for total number of events for use in loop ranges
        self.wrongPerm = np.ndarray((numEvents, 2, self.min_dRgenFinal.shape[1])) # stores the incorrect permutations
        self.allPerm   = np.ndarray((numEvents, self.wrongPerm.shape[1] + 1, 4))  # stores correct and incorrect permutations for each event
        ...
        for event in tqdm(range(numEvents)): # store correct and incorrect permutations in an array with all permutations
    ...
    def invMassCalc(self, ret = False, verbose = False):
        ...
        for event in tqdm(range(num)): # Loop over events
            ...
            A0_c = np.copy(self.min_dRgenFinal[event, 0:2]) # (1 x 2 row vector) for the first correct pair of muons (pair A0) 
            A1_c = np.copy(self.min_dRgenFinal[event, 2:4]) # (1 x 2 row vector) for the second correct pair of muons (pair A1)
            
            # Incorrect permutation indices
            A0_w0 = np.copy(self.wrongPerm[event, 0, 0:2]) # (1 x 2 row vector) for the first wrong pair of muons (pair A0), first incorrect permutation
            A1_w0 = np.copy(self.wrongPerm[event, 0, 2:4]) # (1 x 2 row vector) for the second wrong pair of muons (pair A1), first incorrect permutation
            
            A0_w1 = np.copy(self.wrongPerm[event, 1, 0:2]) # (1 x 2 row vector) for the first wrong pair of muons (pair A0), second incorrect permutation
            A1_w1 = np.copy(self.wrongPerm[event, 1, 2:4]) # (1 x 2 row vector) for the second wrong pair of muons (pair A1), second incorrect permutation

            indicesA_c  = np.column_stack((A0_c, A1_c))   # stack correct indices to make 1 x 4 row vector
            indicesA_c  = indicesA_c.astype(int)          # convert to int
            
            indicesA_w0 = np.column_stack((A0_w0, A1_w0)) # stack indices of first incorrect permutation to make 1 x 4 row vector
            indicesA_w0 = indicesA_w0.astype(int)
            
            indicesA_w1 = np.column_stack((A0_w1, A1_w1)) # stack indices of second incorrect permutation to make 1 x 4 row vector
    ...
    def dR_diMu(self, ret = False, verbose = False):
        ...
        for event in tqdm(range(numEvents)):
            ## pull out the indices of the paired muons and then stack to make a 1 x 4 np.array
            # correct dR A0
            A0_c       = np.copy(self.min_dRgenFinal[event, 0:2]) # pull indices of first pair (1 x 2 np.array)
            A1_c       = np.copy(self.min_dRgenFinal[event, 2:4]) # pull indices of second pair (1 x 2 np.array)
            ...
            A0_w0       = np.copy(self.wrongPerm[event, 0, 0:2]) # pull indices of first pair (1 x 2 np.array)
            A1_w0       = np.copy(self.wrongPerm[event, 0, 2:4]) # pull indices of second pair (1 x 2 np.array)
            indicesA_w0 = np.column_stack((A0_w0, A1_w0))  # stack to make 1 x 4 np.array
            indicesA_w0 = indicesA_w0.astype(int)          # set to int because these are indices
            
            # incorrect permutation 1
            A0_w1       = np.copy(self.wrongPerm[event, 1, 0:2]) # pull indices of first pair (1 x 2 np.array)
            A1_w1       = np.copy(self.wrongPerm[event, 1, 2:4]) # pull indices of second pair (1 x 2 np.array)
            indicesA_w1 = np.column_stack((A0_w1, A1_w1))  # stack to make 1 x 4 np.array
            indicesA_w1 = indicesA_w1.astype(int)          # set to int because these are indices

            ## difference in phi between the first two and second two muons
            # correct dPhi
            self.dPhi[event, 0, 0] = self.selMu_phiFinal[event, indicesA_c[0, 0]]  - self.selMu_phiFinal[event, indicesA_c[1, 0]] # dPhi of first pair of muons for correct pair 
            self.dPhi[event, 0, 1] = self.selMu_phiFinal[event, indicesA_c[0, 1]]  - self.selMu_phiFinal[event, indicesA_c[1, 1]] # dPhi of second pair of muons for correct pair 
            
            # incorrect dPhi 0
            self.dPhi[event, 1, 0] = self.selMu_phiFinal[event, indicesA_w0[0, 0]] - self.selMu_phiFinal[event, indicesA_w0[1, 0]] # dPhi of first pair of muons for first incorrect pair 
            self.dPhi[event, 1, 1] = self.selMu_phiFinal[event, indicesA_w0[0, 1]] - self.selMu_phiFinal[event, indicesA_w0[1, 1]] # dPhi of second pair of muons for first incorrect pair
            
            # incorrect dPhi 0
            self.dPhi[event, 2, 0] = self.selMu_phiFinal[event, indicesA_w1[0, 0]] - self.selMu_phiFinal[event, indicesA_w1[1, 0]] # dPhi of first pair of muons for second incorrect pair 
            self.dPhi[event, 2, 1] = self.selMu_phiFinal[event, indicesA_w1[0, 1]] - self.selMu_phiFinal[event, indicesA_w1[1, 1]] # dPhi of second pair of muons for second incorrect pair

            ## correct dR A0
            self.diMu_dR[event, 0, 0] = sqrt(pow(self.selMu_etaFinal[event, indicesA_c[0, 0]] - self.selMu_etaFinal[event, indicesA_c[1, 0]],2)+ pow(self.dPhi[event,0,0], 2)) # dR of first pair of muons for correct pair 
            ## correct dR A1
            self.diMu_dR[event, 0 ,1] = sqrt(pow(self.selMu_etaFinal[event, indicesA_c[0, 1]] - self.selMu_etaFinal[event, indicesA_c[1, 1]],2)+ pow(self.dPhi[event,0,1], 2))  # dR of second pair of muons for correct pair 

            ## wrong0 dR A0
            self.diMu_dR[event, 1, 0] = sqrt(pow(self.selMu_etaFinal[event, indicesA_w0[0, 0]] - self.selMu_etaFinal[event, indicesA_w0[1, 0]],2)+ pow(self.dPhi[event,1,0], 2)) # dR of first pair of muons for first incorrect pair 
            ## wrong0 dR A1
            self.diMu_dR[event, 1, 1] = sqrt(pow(self.selMu_etaFinal[event, indicesA_w0[0, 1]] - self.selMu_etaFinal[event, indicesA_w0[1, 1]],2)+ pow(self.dPhi[event,1,1], 2)) # dR of second pair of muons for first incorrect pair 

            ## wrong1 dR A0
            self.diMu_dR[event, 2, 0] = sqrt(pow(self.selMu_etaFinal[event, indicesA_w1[0, 0]] - self.selMu_etaFinal[event, indicesA_w1[1, 0]],2)+ pow(self.dPhi[event,2,0], 2)) # dR of first pair of muons for second incorrect pair 
            ## wrong1 dR A1
            self.diMu_dR[event, 2, 1] = sqrt(pow(self.selMu_etaFinal[event, indicesA_w1[0, 1]] - self.selMu_etaFinal[event, indicesA_w1[1, 1]],2)+ pow(self.dPhi[event,2,1], 2)) # dR of second pair of muons for second incorrect pair 
        
        if ret:   
            return self.diMu_dR, self.dPhi

    def fillFinalArray(self, perm = True, diMu_dRBool = True, save = False, pandas = False, ret = True, verbose = False):
        '''
        ...
        if save and not pandas:
            print_error("Cannot save the final array of data unless it is converted to a pandas dataframe! Exiting...\n\n")
            sys.exit()
        ...
        if diMu_dRBool == True:
            ...
            dataframe = np.ndarray((self.min_dRgenFinal.shape[0], 3, 24)) #3199 * 3 * 22
        else:
            dataframe = np.ndarray((self.selMu_ptFinal.shape[0], self.invariantMass.shape[1], 19))
        #print(dataframe.shape)
        
        if perm == False and diMu_dRBool == True: # exclude permutations (only correct pair), onclude dR and dPhi for each paired muon
        ...
        if perm == True and diMu_dRBool == False: # include all permutations, exclude dR and dPhi for each paired muon
            for event in tqdm(range(self.selMu_ptFinal.shape[0])):     # loop over each event
                for permutation in range(self.invariantMass.shape[1]): # loop over each of the permutations
        ...
        if perm == True and diMu_dRBool == True: # include all permutations, dR, and dPhi for each paired muon
        ...
        self.dataframe_shaped = np.reshape(dataframe, (self.selMu_ptFinal.shape[0] * self.invariantMass.shape[1], 24))

        if pandas:
            total_df = pd.DataFrame(dataframe_shaped, columns = ["selpT0", "selpT1", "selpT2", "selpT3", "selEta0", "selEta1", "selEta2", "selEta3",\
                       "selPhi0", "selPhi1", "selPhi2", "selPhi3", "selCharge0", "selCharge1", "selCharge2", "selCharge3", "dPhi0", "dPhi1","dRA0",\
                        "dRA1", "event", "invMassA0", "invMassA1", "pair"])
    ...
    def fillAndSort(self, save = False, ret = True, verbose = False, custom_dir = None):
        ...
        if self.dataset == "mc" or self.ml_met:
            if save:
                total_df = pd.DataFrame(self.dataframe_shaped, columns = ["selpT0", "selpT1", "selpT2", "selpT3", "selEta0", "selEta1", "selEta2", "selEta3",\
