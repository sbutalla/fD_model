...
class histogram:
    def __init__(self, dataset, sigDir=None):
        ...
        if self.dataset == "sig":
            if sigDir is None:
                print(
                    colors.YELLOW
                    + "A list of signal directories is required (without the preceding parent directories, e.g., sigDir = [MZD_150_55, MZD_95_15]). Exiting...\n"
                    + colors.ENDC
                )
        ...
        if self.dataset == "bkg":
            if csv_type == "single":
                ...
                for ii in range(len(plot_order)):
                    temp_file = (
                        data_directory
                        + plot_order[ii]
                        + "/single_correct_pair_%s.csv" % plot_order[ii]
                    )
            ...
            if csv_type == "single":
                ...
                for dataset in range(len(self.sig_dir)):
                    temp_file = (
                        data_directory
                        + self.sig_dir[dataset]
                        + "/single_correct_pair_%s.csv" % self.sig_dir[dataset]
                    )
                    single_correct_pair_all[self.sig_dir[dataset]] = pd.read_csv(
                        temp_file
                    )
    ...
    def import_correctPairs(self, dictionary):
        if self.dataset == "bkg":
            ...
            self.invMass_bkg = {}
            for key in keyList:
                self.invMass_bkg[key] = {
                    self.diMuonKey[0]: None,
                    self.diMuonKey[1]: None,
                }
                for pair in range(2):
                    self.invMass_bkg[key][self.diMuonKey[pair]] = dictionary[key][
                        self.diMuonKey[pair]
                    ].to_numpy()
            ...
            for key in self.sig_dir:
                self.invMass_sig[key] = {
                    self.diMuonKey[0]: None,
                    self.diMuonKey[1]: None,
                }
                for pair in range(2):
                    self.invMass_sig[key][self.diMuonKey[pair]] = dictionary[key][
                        self.diMuonKey[pair]
                    ].to_numpy()
    ...
    def create_hist(self, sf=True, verbose=False):
        if self.dataset == "bkg":
            ...
            for key in plot_order:
                self.histData_bkg[key] = {
                    self.diMuonKey[0]: None,
                    self.diMuonKey[1]: None,
                }

                for pair in self.diMuonKey:
                    temp_hist, temp_bins = np.histogram(
                        self.invMass_bkg[key][pair],
                        bins=numBins,
                        range=hist_range,
                        weights=None,
                        density=None,
            ...
            if sf:
                for key in plot_order:
                    for pair in self.diMuonKey:
                        if verbose:
                            print(
                                "Applying scale factor of %f to %s\n\n"
                                % (scale_factors_bkg[key], key)
                            )

                        self.histData_bkg[key][pair]["hist"] = (
                            self.histData_bkg[key][pair]["hist"]
                            * scale_factors_bkg[key]
                        )
        elif self.dataset == "sig":
            self.histData_sig = {}

            for key in self.sig_dir:
                self.histData_sig[key] = {
                    self.diMuonKey[0]: None,
                    self.diMuonKey[1]: None,
                }
                for pair in self.diMuonKey:
                    temp_hist, temp_bins = np.histogram(
                        self.invMass_sig[key][pair],
                        bins=numBins,
                        range=hist_range,
                        weights=None,
                        density=None,
                    ...
                    if sf:
                        ...
                        self.histData_sig[key][pair]["hist"] = (
                            self.histData_sig[key][pair]["hist"]
                            * scale_factors_sig[key]
                        )
    ...
    def create_hist_2d(self, sf=True, verbose=False):
        if self.dataset == "bkg":
            ...
            for key in plot_order:
                temp_hist, temp_x_edge, temp_y_edge = np.histogram2d(
                    self.invMass_bkg[key][self.diMuonKey[0]],
                    self.invMass_bkg[key][self.diMuonKey[1]],
                    bins=numBins,
                )

                self.histData_2d_bkg[key] = {
                    "hist": temp_hist,
                    "x_edge": temp_x_edge,
                    "y_edge": temp_y_edge,
                }
                ...
                if sf:
                    ...
                    self.histData_2d_bkg[key]["hist"] = (
                        self.histData_2d_bkg[key]["hist"].T * scale_factors_bkg[key]
                    )
            ...
            for key in self.sig_dir:
                temp_hist, temp_x_edge, temp_y_edge = np.histogram2d(
                    self.invMass_sig[key][self.diMuonKey[0]],
                    self.invMass_sig[key][self.diMuonKey[1]],
                    bins=numBins,
                )

                self.histData_2d_sig[key] = {
                    "hist": temp_hist,
                    "x_edge": temp_x_edge,
                    "y_edge": temp_y_edge,
                }
                ...
                if sf:
                    ...
                    self.histData_2d_sig[key]["hist"] = (
                        self.histData_2d_sig[key]["hist"].T * scale_factors_sig[key]
                    )
        ...
        if self.dataset == "bkg":
            ...
            for pair in self.diMuonKey:
                ...
                for key in plot_order:
                    ...
                    if cnt == 0:
                        ...
                        for ii in range(cnt):
                            temp_bottom += self.histData_bkg[plot_order[ii]][pair][
                                "hist"
                            ]
                    ...
                    if cnt == 0:
                        plt.bar(
                            self.histData_bkg[key][pair]["bins"][:-1],
                            self.histData_bkg[key][pair]["hist"],
                            width=3.5,
                            label=temp_string,
                            edgecolor="black",
                ...
                plt.xlabel(
                    r"$m_{\mu\mu_{%i}}$ [GeV]" % mu_cnt, fontsize=30, loc="right"
                )
                plt.xlim(hist_range)
                plt.ylim(0, 15)
            ...
            for key in self.sig_dir:
                ...
                for pair in self.diMuonKey:
                    ...
                    plt.bar(
                        self.histData_sig[key][pair]["bins"][:-1],
                        self.histData_sig[key][pair]["hist"],
                        width=3.5,
                        label="MC Signal Data\n"
                        + r"$m_{Z_{D}} = %s$,"
                        + "\n"
                        + r"$m_{f_{D1}} = %s$" % (key.split("_")[1], key.split("_")[2]),
                        edgecolor="black",
                        color="darkred",
                        zorder=3,
                    )
                    ...
                    if save:
                        plt.savefig(
                            data_directory
                            + key
                            + "/signal_%s_hist_diMu_%s.pdf" % (key, pair)
                        )
    ...
    def plot2Dhist(self, save=True):
        ...
        if self.dataset == "bkg":
            ...
            temp_key = list(self.histData_2d_bkg.keys())[
                0
            ]  # to index array when setting extent for 2D histogram
            ...
            # the 2D plot:
            plot2d = ax2d.imshow(
                self.total_2d_hist_bkg,
                interpolation="none",
                origin="lower",
                extent=[
                    self.histData_2d_bkg[temp_key]["x_edge"][0],
                    self.histData_2d_bkg[temp_key]["x_edge"][-1],
                    self.histData_2d_bkg[temp_key]["y_edge"][0],
                    self.histData_2d_bkg[temp_key]["y_edge"][-1],
                ],
                aspect="auto",
                cmap=cm.plasma,
            )
            ...
            axHisty.barh(
                self.histData_2d_bkg[temp_key]["y_edge"][:-1], yhist, 3.5, zorder=3
            )
            ...
            for key in self.sig_dir:
                ...
                # the 2D plot:
                plot2d = ax2d.imshow(
                    self.total_2d_hist_sig[key],
                    interpolation="none",
                    origin="lower",
                    extent=[
                        self.histData_2d_sig[key]["x_edge"][0],
                        self.histData_2d_sig[key]["x_edge"][-1],
                        self.histData_2d_sig[key]["y_edge"][0],
                        self.histData_2d_sig[key]["y_edge"][-1],
                    ],
                    aspect="auto",
                    cmap=cm.plasma,
                )
                ...
                axHistx.grid(zorder=0)
                axHistx.bar(
                    self.histData_2d_sig["x_edge"][:-1], xhist, width=3.5, zorder=3
                )
                ...
                # use barh instead of bar here because we want a horizontal histogram
                axHisty.barh(
                    self.histData_2d_sig[key]["y_edge"][:-1], yhist, 3.5, zorder=3
                )
                axHisty.grid(zorder=0)
...
def ratio_hist(hist_2D_total_bkg, hist_2D_total_sig, histData_2d_sig, save=True):
    ...
    plot2d = ax2d.imshow(
        ...
        origin="lower",
        extent=[
            histData_2d_sig["x_edge"][0],
            histData_2d_sig["x_edge"][-1],
            histData_2d_sig["y_edge"][0],
            histData_2d_sig["y_edge"][-1],
        ],
        aspect="auto",
        cmap=cm.plasma,
    )
