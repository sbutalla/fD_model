...
def sort_pd(dRgenFinal, allPerm, selMu_ptCut, selMu_etaCut, selMu_phiCut, selMu_chargeCut, invariantMass, dPhi, diMu_dR):
...
def shape_pd(X_train, X_test, y_train, y_test):
    ...
    loaded_model_pd = joblib.load(filename)
    ...
    ax = correct_pair['invMassA0'].plot.hist(bins=100, alpha=0.8, range=(0, 100), color='darkred' , ax=axess[0,0])
    ax.set_xlabel(r'$m_{\mu\mu_{1}}$[GeV]', loc='right')
    ...
    ax =  correct_pair['invMassA1'].plot.hist(bins=100, alpha=0.8,  range=(0, 100), color='darkred' , ax=axess[0,1])
    ax.set_xlabel(r'$m_{\mu\mu_{2}}$[GeV]', loc='right')
    ax.set_ylabel('Numbrt of Events', loc='top')
    # ax.set_xlim(0, 30)
    ax.set_title('Correct Pair')

    ax =  wrong_pair['invMassA0'].plot.hist(bins=100, alpha=0.8, range=(0, 200),color='darkred', ax=axess[1,0])
    ax.set_xlabel(r'$m_{\mu\mu_{1}}$[GeV]', loc='right')
    ax.set_ylabel('Numbrt of Events', loc='top')
    # ax.set_xlim(0, 180)
    ax.set_title('Wrong Pair')

    ax =  wrong_pair['invMassA1'].plot.hist(bins=100, alpha=0.8, range=(0, 200), color='darkred',  ax=axess[1,1])
    ax.set_xlabel(r'$m_{\mu\mu_{2}}$[GeV]', loc='right')
    # ax.set_xlim(0, 180)
    ...
    return sorted_X, correct_pair, wrong_pair, loaded_model_pd
